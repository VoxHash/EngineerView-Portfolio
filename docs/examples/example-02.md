...
- Plausible Analytics account (optional)
...
export default function AdvancedAnalytics() {
  useEffect(() => {
    ...
    const trackProjectView = (projectName: string) => {
      trackEvent('Project View', { project: projectName });
    };

    const trackContactForm = (formType: string) => {
      trackEvent('Contact Form', { form: formType });
    };
    ...
    (window as any).trackProjectView = trackProjectView;
...
1. **Environment Variables in Vercel**:
   ...
   PLAUSIBLE_DOMAIN=your-vercel-app.vercel.app
   PLAUSIBLE_API_KEY=your-plausible-api-key
   RESEND_API_KEY=re_your_resend_api_key
   NEXT_PUBLIC_PLAUSIBLE_DOMAIN=your-vercel-app.vercel.app
   ```
...
1. **Plausible Analytics**: Track visitor behavior
